...
namespace ifd {
	...
	bool PathBox(const char* label, std::filesystem::path& path, char* pathBuffer, ImVec2 size_arg) {
		...
		if (!(*state & 0b001)) {
			...
			if (!anyOtherHC && clicked) {
				strcpy(pathBuffer, reinterpret_cast<const char*>(path.c_str()));
				*state |= 0b001;
	...
	FileDialog::FileData::FileData(const std::filesystem::path& path) {
		...
		struct stat attr;
		stat(reinterpret_cast<const char*>(path.c_str()), &attr);
	...
	{
		auto itr = std::ranges::find(m_favorites, m_currentDirectory.string());
	...
	void FileDialog::m_renderContent()
	{
		...
		// "icon" view
			...
			for (auto& entry : m_content) {
				...
				if (FileIcon(filename.c_str(), isSelected, entry.HasIconPreview ? reinterpret_cast<ImTextureID>(entry.IconPreview) : reinterpret_cast<ImTextureID>(m_getIcon(entry.Path)), ImVec2(32 + 16 * m_zoom, 32 + 16 * m_zoom), entry.HasIconPreview, entry.IconPreviewWidth, entry.IconPreviewHeight)) {
					std::error_code ec;
					bool isDir = std::filesystem::is_directory(entry.Path, ec);
